...
function StatusBadge({ status }: { status: BatchStatus }) {
  const statusConfig = {
    ...
    CANCELED: { label: "Canceled", icon: PauseCircle, variant: "outline" as const },
  };
  ...
  return (
    <div className="border rounded-md">
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead onClick={() => handleSort(SortField.NAME)} className="w-[200px] cursor-pointer">
              <div className="flex items-center gap-1">
                Name
                {getSortDirection(SortField.NAME) === "asc" ? (
                  <ArrowUp className="w-4 h-4" />
                ) : getSortDirection(SortField.NAME) === "desc" ? (
            ...
            <TableHead onClick={() => handleSort(SortField.MODEL)} className="cursor-pointer">
              <div className="flex items-center gap-1">
                Model
                {getSortDirection(SortField.MODEL) === "asc" ? (
                  <ArrowUp className="w-4 h-4" />
                ) : getSortDirection(SortField.MODEL) === "desc" ? (
                  <ArrowDown className="w-4 h-4" />
                ) : (
                  <ArrowUpDown className="w-4 h-4" />
                )}
            ...
            <TableRow key={batch.id}>
                ...
                {batch.description && (
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p className="max-w-xs">{batch.description}</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                )}
              ...
              <TableCell>
                <span className="font-mono text-xs">
                  {batch.model.split("-").slice(0, 3).join(" ")}
                </span>
              </TableCell>
              <TableCell>
              ...
              <TableCell className="text-muted-foreground">
                {batch.completedAt 
                  ? format(new Date(batch.completedAt), "MMM d, yyyy") 
                  : "-"}
              </TableCell>
              <TableCell>
              ...
              <TableCell className="text-right">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem onClick={() => batch.exportBatch()}>
