...
export async function POST(req: Request) {
    try {
        ...
        if (!signature || !timestamp) {
            console.error(
                `Missing webhook signature headers for webhook ${webhookId}`,
            );
        ...
        if (!batchId) {
            console.error(
                `Missing batch ID in webhook payload for webhook ${webhookId}`,
            );
        ...
        if (!batch) {
            console.error(
                `Batch not found for Anthropic ID: ${batchId} (webhook ${webhookId})`,
            );
        ...
        // Log webhook event
        console.log(
            `Processing webhook ${webhookId} for batch ${batch.id} (${batch.name}): ${payload.type}`,
        );

        // Verify the webhook signature using either the user's webhook secret or the default one
        const webhookSecret =
            batch.user.anthropicWebhookSecret ||
            process.env.ANTHROPIC_WEBHOOK_SECRET;
        ...
        if (webhookSecret) {
            ...
            signatureValid = anthropicClient.verifyWebhookSignature(
                signature,
                timestamp,
                body,
            );
        ...
        try {
            switch (payload.type) {
                ...
                case "batch.in_progress":
                    ...
                    console.log(
                        `Successfully processed webhook ${webhookId} for batch ${batch.id}`,
                    );
                    return new Response("OK");
                default:
                    console.warn(
                        `Unhandled webhook type: ${payload.type} for webhook ${webhookId}`,
                    );
                    return new Response("OK");
            }
        } catch (error) {
            console.error(
                `Error processing webhook ${webhookId} for batch ${batch.id}:`,
                error,
            );
...
async function handleBatchUpdate(payload: any, batch?: any) {
    ...
    console.log(
        `Updating batch ${batch.id} status to ${mapAnthropicStatus(status)}`,
    );
