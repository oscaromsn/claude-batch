...
import type { Completion, CompletionStatus } from "@prisma/client";
 ...
 */
export async function updateBatchCompletionStats(
    batchId: string,
): Promise<void> {
    ...
    const inProgressCount = await countCompletionsByStatus(
        batchId,
        "IN_PROGRESS",
    );
    ...
    const totalCount =
        completedCount + failedCount + pendingCount + inProgressCount;
    ...
    const inputTokens = completions.reduce(
        (sum, comp) => sum + (comp.inputTokens || 0),
        0,
    );
    const outputTokens = completions.reduce(
        (sum, comp) => sum + (comp.outputTokens || 0),
        0,
    );
